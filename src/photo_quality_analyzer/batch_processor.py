"""
Batch Processor for Photo Quality Analyzer
...
from .analyzer import PhotoQualityAnalyzer
...
class BatchResult:
    ...
    Attributes:
        ...
        scores: List of (photo_path, QualityScore) tuples for successful analyses
        errors: List of (photo_path, error_message) tuples for failed analyses
    """
    total_photos: int
    successful: int
    failed: int
    scores: List[Tuple[str, QualityScore]]
...
def _process_single_photo(photo_path: str, config_dict: dict) -> Tuple[str, Optional[QualityScore], Optional[str]]:
    """
    ...
    Returns:
        Tuple of (photo_path, QualityScore or None, error_message or None)
    """
    try:
        ...
        # Apply config overrides from dict
        if 'weights' in config_dict:
            config.weights.sharpness = config_dict['weights'].get('sharpness', 0.6)
            config.weights.exposure = config_dict['weights'].get('exposure', 0.4)
...
class BatchProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            chunk_size: Photos per chunk (defaults to config value)
        """
        self.config = config or QualityAnalyzerConfig()
        self.num_workers = num_workers or self.config.performance.default_num_workers
        self.chunk_size = chunk_size or self.config.performance.default_batch_size
    ...
    def process_batch_chunked(
        ...
        Examples:
            >>> # Process 100,000 photos in chunks of 500
            >>> processor = BatchProcessor(chunk_size=500)
            >>> result = processor.process_batch_chunked(large_photo_list)
        """
        total_photos = len(photo_paths)
        ...
        logger.info(f"Using chunk size of {self.chunk_size} and {self.num_workers} workers")

        all_scores: List[Tuple[str, QualityScore]] = []
        ...
        for i in range(0, total_photos, self.chunk_size):
            ...
            total_chunks = (total_photos + self.chunk_size - 1) // self.chunk_size
    ...
    def validate_paths(self, photo_paths: List[str]) -> Tuple[List[str], List[Tuple[str, str]]]:
        """
        ...
        for path in photo_paths:
            ...
            if not path_obj.exists():
                ...
                invalid_paths.append((path, "Not a file"))
            elif not path_obj.stat().st_size > 0:
                invalid_paths.append((path, "File is empty"))
            else:
